...
type WebAPIConfiguration struct {
	...
	Web struct {
		...
		WriteTimeout    time.Duration `conf:"default:3600s"`
		ShutdownTimeout time.Duration `conf:"default:5s"`
	}
	Debug bool
	DB    struct {
		Filename string `conf:"default:/tmp/whaledeployer/database.db"`
	}
	Data struct {
		KeysPath string `conf:"default:/tmp/whaledeployer/keys"`
	}
}
